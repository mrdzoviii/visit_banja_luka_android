...
public class EventsFragment extends Fragment implements IFragment {
    ...
    private RecyclerView.Adapter mAdapter = new ItemAdapter(items, AppController.mainActivity,this);
    ...
    }
    {
       // load();
        if(!pulled){
            pulled=true;
            new ItemsTask(items,mAdapter,Item.EVENT).execute(AppController.mainActivity);
        }
    }
    ...
    private void load(){
        recyclerView = AppController.mainActivity.findViewById(R.id.rvEvent);
